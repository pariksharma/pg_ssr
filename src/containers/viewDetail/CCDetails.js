...
export default function CCDetails() {
  ...
  const [childCourseDetails, setChildCourseDetails] = useState();
  ...
  useEffect(() => {
    if (typeof searchParam !== "undefined") {
      const [value1, value2] = searchParam.get('id').split('_');
      getParentDetail(value1)
      getCourseDetail(value1, value2);
      setChildId(value2)
  ...
  const getParentDetail = async (value) => {
    setIsLoading(true)
  ...
  const getCourseDetail = async (value1, value2) => {
    const formData = new FormData();
    // if(value.includes('_')){
    //     const [value1, value2] = value.split('_');
    //   const stringWithoutUnderscore = value.replace(/_/g, '');
    formData.append("course_id", value2);
    formData.append("parent_id", value1);
    // }
    // else{
    //   formData.append("course_id", value);
                  ...
                  <div className="pg-breadcrum">
                    <nav aria-label="breadcrumb">
                      <ol className="breadcrumb mb-2 cursor" >
                        ...
                        {courseDetails && <li className="breadcrumb-item active" onClick={() => navigate(`/view_details?course_id=${parentId}_`)}>{courseDetails.course_detail.title}</li>}
                        {childCourseDetails && <li className="breadcrumb-item highlight">{childCourseDetails.course_detail.title}</li>}
                      </ol>
                    </nav>
                    ...
                    <div className="d-flex align-items-center justify-content-between">
                      <div className="mt-2">
                        <h1>
                          {childCourseDetails && childCourseDetails.course_detail.title}
                        </h1>
                      </div>
                          ...
                          tilesData.map((item, i) => {
                            return (
                              ...
                              remainingTilesData && remainingTilesData.length > 0 ? <DisplayContent value={remainingTilesData}/> :<div className='text-center'> <NoDataFound/> </div>
                                ...
                                {
                                  item.type == "content" && <Content value={item} resetRef={resetContentLayerRef} course_id={childId} tabName={item.tile_name} is_purchased={courseDetails?.course_detail.is_purchased} mrp={courseDetails?.course_detail.mrp} />
                                }
                                {/* {
