...
const Test_icon2 = "/assets/images/Test.png";

export default function SubjectiveLayer3({ value }) {
  ...
  const handleMarks = (data) => {
    if (Number(value.result_date) < Number(Date.now())) {
      setOpenModal(true);
      // setModalValue(`<h6>Total Marks : ${data.total_marks}</h6><br/><h6>Total Questions : ${data.total_questions}</h6><br/><h6> Correct: ${data.correct_count}</h6> <br/><h6>Incorrect : ${data.incorrect_count} </h6>`);
      ...
      setOpenModal(true);
      setModalValue(`<h6>Result Not Declared Yet!!</h6>`)
    }
  }
