...
const Test_icon = "/assets/images/icons/Test_objective.png";
...
export default function Daily_Assignment({ value, tabName, is_purchased, mrp, resetRef }) {
  const [layer1Data, setLayer1Data] = useState();
  const [dailyAssUrl, setDailyAssUrl] = useState()
  ...
  const handleOpenDailyASS = async (item) => {
    if (isLogin) {
      if (item.is_locked == "0" || is_purchased == "1") {
        setDailyAssUrl(item.file_url);
        setIsModalOpen(true);
      } else {
  ...
  return (
    <>
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setDailyAssUrl(); }}>
        {dailyAssUrl ? <img className="image_modal" src={dailyAssUrl} /> : 'NO Image'}
      </Modal>
      ...
      {showLayer == "layer3"
        ...
        layer3Data.list.map((item, i) => {
          return (
            <div
              className="col-md-12 pg-tabs-description"
              key={i}
              onClick={() => handleOpenDailyASS(item)}
            >
              <div className="sourse-conten tabs-deschovr d-flex align-items-center rounded shadow">
                <div
                ...
                {is_purchased === "1" && (
                  <div className="btnsalltbb" style={{ width: "50%" }}>
                    ...
                    <button className="btn">Booklet</button>

                    <button className="btn">Marks</button>
                  </div>
                )}
                <div className="pg-sb-topic pe-2">
                  <div className="videoimage text-center">
                    {" "}
